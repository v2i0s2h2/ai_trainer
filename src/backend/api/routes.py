...
class ExerciseResponse(BaseModel):
    ...
    weight_progression: Optional[WeightProgression] = (
        None  # Weight progression guidance
    )
...
async def get_today_stats(db: Session = Depends(get_db)):
    ...
    if all_workouts:
        ...
        for workout in sorted_workouts:
            workout_date = (
                workout.date.date() if hasattr(workout.date, "date") else workout.date
            )
            if workout_date == check_date or workout_date == check_date - timedelta(
                days=1
            ):
        ...
        most_recent_date = (
            sorted_workouts[0].date.date()
            if hasattr(sorted_workouts[0].date, "date")
            else sorted_workouts[0].date
        )
    ...
    try:
        ...
        for achievement in all_achievements:
            ...
            if achievement.id in unlocked_achievement_ids:
                ...
                if unlock_date:
                    date_str = (
                        unlock_date.strftime("%b %d")
                        if hasattr(unlock_date, "strftime")
                        else str(unlock_date)
                    )
                else:
                    date_str = ""
        ...
        return {"unlocked": [], "locked": [], "total": 0, "unlocked_count": 0}
...
async def get_user_profile(
    ...
    from src.backend.database.models import UserAchievement, Workout
    ...
    preferences = UserPreferences(
        notifications_enabled=True, units="metric", language="en"
    )
