...
<div class="profile-page">
	...
	{:else if error}
		<div class="error-state">
			<p>❌ {error}</p>
			<button class="retry-btn" on:click={() => profileStore.loadProfile()}>
				Retry
			</button>
		</div>
	{:else if profile}
		...
		<div class="profile-card">
			<div class="avatar-section">
				<div class="avatar">
					<span class="avatar-initials">{getInitials(profile.name)}</span>
				</div>
			...
			{:else if achievements && achievements.unlocked && achievements.unlocked.length > 0}
				<div class="achievements-grid">
					{#each achievements.unlocked.slice(0, 6) as achievement}
						<div class="achievement-badge unlocked" title={achievement.name}>
